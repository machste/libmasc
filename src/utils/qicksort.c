...
static void sift(unsigned char *head, size_t width, qs_cmp_cb cmp, void *arg,
...
static void trinkle(unsigned char *head, size_t width, qs_cmp_cb cmp, void *arg,
...
void quicksort(void *base, size_t nel, size_t width, qs_cmp_cb cmp, void *arg)
{
    ...
    while(pshift != 1 || p[0] != 1 || p[1] != 0) {
        if(pshift <= 1) {
            ...
            p[0] ^= 7;
            shr(p, 1);
            trinkle(head - lp[pshift] - width, width, cmp, arg,
                    p, pshift + 1, 1, lp);
            shl(p, 1);
            p[0] |= 1;
